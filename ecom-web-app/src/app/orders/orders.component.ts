...
export class OrdersComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http.get("http://host.docker.internal:8888/BILLING-SERVICE/fullBill/search/"+this.customerId,{headers:new HttpHeaders({"Authorization":"Bearer "+token})}).subscribe(
      {
        next:(data)=>{
          console.log(data)
        this.orders=data;
